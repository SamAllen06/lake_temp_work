...
def graph(
    ...
    ax.set_zlabel(data.var_name)

    ax.view_init(elevation, azimuth, roll)

    if not dir_path.exists():
        dir_path.mkdir(parents=True)
    fig.savefig(str(dir_path)+f'/e{elevation}a{azimuth}r{roll}.png')
...
def _get_var_data(data: GraphData, dataset: Dataset) -> npt.NDArray:
    ...
    for fixed_dim in data.fixed_dims:
        slices[fixed_dim.mapped_dim_name] = fixed_dim.value
...
def _get_axis_values(axis: AxisInfo, dataset: Dataset) -> tuple[npt.NDArray, str]:
    ...
    if axis.dim_name == 'sample_index':
        mapped_var = dataset.variables[list(axis.mapped_var_name.keys())[0]]
        mapped_var_name = ', '.join(key+': '+value for key, value in axis.mapped_var_name.items()) 
        name = f"{axis.dim_name} ({mapped_var_name})"
        sample_index_array = range(0, len(mapped_var[axis_slice]))
        return sample_index_array, name

    # elif axis.mapped_var_name and axis.dim_name != 'time':
    #     mapped_var = dataset.variables[list(axis.mapped_var_name.keys())[0]]
    #     name = f"{axis.dim_name} ({axis.mapped_var_name})"
